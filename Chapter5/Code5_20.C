//code with fit on data without assigning error
// Script fitnoerr
{
 ...
 //  Canvas_1->SetGridy();
  ...
  pippo->Draw("ap");
  pippo->Fit("ff");//***
  float mean,sigma,mean2,sigma2,chi,ndf;
  ...
  mean=ff->GetParameter(0);
  sigma=ff->GetParError(0);
  mean2=ff->GetParameter(1);
  sigma2=ff->GetParError(1);
  chi=ff->GetChisquare();
  ndf=ff->GetNDF();
  leg->AddEntry(pippo,Form("#chi^{2} / ndf = %6.2f / %3.0f",chi,ndf),"");
  leg->AddEntry(pippo,Form("A=%5.2f #pm %5.2f",mean,sigma),"");
  leg->AddEntry(pippo,Form("n2=%5.2f #pm %5.2f",mean2,sigma2),"");
  leg->Draw();
